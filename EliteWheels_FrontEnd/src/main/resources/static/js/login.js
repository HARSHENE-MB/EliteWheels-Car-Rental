...
document.getElementById('sendCodeBtn').addEventListener('click', async function () {
    ...
    if (phoneNumber.length === 10) {
        try {
            const response = await fetch(`http://localhost:8081/api/users/phone/${phoneNumber}`);
            ...
            if (response.ok) {
				...
				Swal.fire({
				  ...
				  html: `No account exists with this phone number.<br><br>
				         <a href="/register" style="color: #3085d6; text-decoration: underline;">Try registering here</a>`,
				  confirmButtonText: 'OK'
				});
        ...
        Swal.fire('Invalid Number', 'Please enter a valid 10-digit phone number', 'error');
    }
});
...
document.getElementById('verifyCodeBtn').addEventListener('click', function () {
    ...
    if (enteredOTP === generatedOTP) {
        Swal.fire({
            ...
            window.location.href = '/profile'; // Thymeleaf route
        });
    } else {
        Swal.fire({
            title: 'Invalid OTP',
    ...
    document.getElementById('resendCodeBtn').addEventListener('click', function(e) {
        e.preventDefault();
        // Normally, the OTP would be regenerated on the server-side and sent to the user
        generatedOTP = generateOTP();
        const phoneNumber = document.getElementById('phoneNumber').value;
        Swal.fire({
            title: 'OTP Resent!',
